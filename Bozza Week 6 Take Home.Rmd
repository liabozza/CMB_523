...
ANSWER 1: <This code assigns the variable res to the function. The function will run if the random chosen value is 125000001.#Note: Seed is where the code will start running every time even if computer is restarted.>
...
for(times_code_has_run in 1:3){ #while(time_code_has_run<4){another option
 start_time<-Sys.time()
...
print(time.taken) #print the average of three runs
for(i in length(time.taken)){
 if("start_time"[i]<"end_time"[i]){
  print[i]
  Average_of_the_three_runs<-mean(time.taken)
 }
  print(Average_of_the_three_runs)
...
<i represents lowercase "a" in a row. j represents the column index of "a".>
...
<In Method 1, for(i in 1...j in 1), starting with 1 instead of (sequencing matrix) means that for the 1000 rows of samples, the loops starts checking for "a" at row 1,col 1, then checks for row 1, col 2, up to 1000. First version's loop ran 1000 times.> 
...
for(i in 1:nrow(sequencing_matrix)){
  for(j in 1:ncol(sequencing_matrix)){
    if(sequencing_matrix[i,j]=="T") 
      counter <- counter + 1     #For each row and column, find a "T" and count it. Add the located T to                                     #the existing counted Ts
    }                                           
    print(counter)
} 
...
#for(i in nrows){
 ...
 # for(j in ncols){
  ...
  #  }} 
   # if(sequencing_matrix[i,j]=="T")
#print(rowCounts(i, value=T))#rowALLs() check to see if value exists/does not exist in each row
#print(colCounts(j, value=T)) #colALLs() analogous to rowALLs()
#for (i in ncol(sequencing_matrix)){
#  print('Count of', i, ':', sum(T==i), '/',sep='')
